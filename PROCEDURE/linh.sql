...
---------0. TimPhong------------------------------
CREATE PROCEDURE TimPhong @TuKhoa varchar(15)
AS
...
CREATE 
--ALTER 
PROCEDURE DSDVDK @MaKH varchar(15)
AS
...
WHERE PDP.MAKH=@MaKH AND cast(PDP.NGAYDI AS DATE)=cast(GETDATE() AS DATE)


--EXEC DSDVDK 'KH606532' 
--SELECT * FROM PHIEUDATPHONG 
--SELECT * FROM PHIEU_DKDV_SP
...
CREATE
--ALTER
PROCEDURE HoaDon @MaKH varchar(15)
AS
...
--
--
----Lấy ra chi tiết đặt phòng và giá ti�?n mỗi phòng
--SELECT CTDP.MAPHONG, P.GIA, PDP.NGAYLAP
--FROM HOADONPHONG HDP JOIN PHIEUDATPHONG PDP ON HDP.MAPDK=PDP.MAPDK 
--JOIN CHITIETDATPHONG CTDP ON CTDP.MAPDK=PDP.MAPDK
--JOIN PHONG P ON P.MAPHONG=CTDP.MAPHONG 
...
--
------Lấy ra tổng ti�?n 
----1. HOADON * PDP (MKH = @MKH và CHECKOUT = �?IỀU KIỆN HÔM NAY) 
----2. PDP * CTDP
----3. CTDP*P --> Tổng ti�?n cho các phòng
----4. Gán vào THANHTIEN (PDP) Gán vào HDP
--
